...
import (
	...
	"basic-go/webook/interaction/integration/startup"
	...
	testCases := []struct {
	...
	}{
		{
			...
			before: func(t *testing.T) {
				...
				err = s.rdb.HSet(ctx, "interaction:test:2",
					"read_cnt", 3).Err()
				assert.NoError(t, err)
			},
			after: func(t *testing.T) {
				...
				cnt, err := s.rdb.HGet(ctx, "interaction:test:2", "read_cnt").Int()
				assert.NoError(t, err)
				assert.Equal(t, 4, cnt)
				err = s.rdb.Del(ctx, "interaction:test:2").Err()
				assert.NoError(t, err)
			},
			biz:      "test",
			bizId:    2,
		...
		},
		{
			...
			after: func(t *testing.T) {
				...
				cnt, err := s.rdb.Exists(ctx, "interaction:test:3").Result()
				assert.NoError(t, err)
		...
		},
		{
			...
			after: func(t *testing.T) {
				...
				cnt, err := s.rdb.Exists(ctx, "interaction:test:4").Result()
				assert.NoError(t, err)
				assert.Equal(t, int64(0), cnt)
			},
			biz:      "test",
	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				cnt, err := s.rdb.HGet(ctx, "interaction:test:2", "like_cnt").Int()
				assert.NoError(t, err)
				assert.Equal(t, 4, cnt)
				err = s.rdb.Del(ctx, "interaction:test:2").Err()
				assert.NoError(t, err)
		...
		},
		{
			...
			after: func(t *testing.T) {
				...
				cnt, err := s.rdb.Exists(ctx, "interaction:test:2").Result()
				assert.NoError(t, err)
				assert.Equal(t, int64(0), cnt)
			},
			biz:      "test",
	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				cnt, err := s.rdb.Exists(ctx, "interaction:test:1").Result()
				assert.NoError(t, err)
				assert.Equal(t, int64(0), cnt)
				// 收藏记录
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				assert.NoError(t, err)
				err = s.rdb.HSet(ctx, "interaction:test:3", "collect_cnt", 10).Err()
				assert.NoError(t, err)
			},
			after: func(t *testing.T) {
				ctx, cancel := context.WithTimeout(context.Background(), time.Second)
				...
				cnt, err := s.rdb.HGet(ctx, "interaction:test:3", "collect_cnt").Int()
				assert.NoError(t, err)
	...
	testCases := []struct {
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				assert.NoError(t, err)
				err = s.rdb.HSet(ctx, "interaction:test:3",
